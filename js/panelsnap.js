import Tweezer from "/js/tweezer.js";
...
export default class PanelSnap {
    constructor(options) {
        ...
        this.scrollEventContainer.addEventListener(
            ...
            passiveIsSupported && { passive: true }
        );
        this.scrollEventContainer.addEventListener(
            "keyup",
            this.onInteractStop,
    ...
    findSnapTarget() {
        ...
        const panelsInViewport = getElementsInContainerViewport(
            this.container,
            this.panelList
        );
        ...
        if (panelsInViewport.length > 1) {
            ...
            const targetIndex =
                deltaY > 0 || deltaX > 0 ? 1 : panelsInViewport.length - 2;
            this.snapToPanel(
                panelsInViewport[targetIndex],
                deltaY < 0,
                deltaX < 0
            );
            return;
    ...
    snapToPanel(panel, toBottom = false, toRight = false) {
        ...
        this.targetScrollOffset = getTargetScrollOffset(
            this.container,
            panel,
            toBottom,
            toRight
    ...
    animationTick(value) {
        const scrollTopDelta =
            this.targetScrollOffset.top - this.currentScrollOffset.top;
        const scrollTop =
            this.currentScrollOffset.top +
            (scrollTopDelta * value) / TWEEN_MAX_VALUE;
        this.scrollContainer.scrollTop = scrollTop;

        const scrollLeftDelta =
            this.targetScrollOffset.left - this.currentScrollOffset.left;
        const scrollLeft =
            this.currentScrollOffset.left +
            (scrollLeftDelta * value) / TWEEN_MAX_VALUE;
